...
class Data_viower extends HTMLElement {
    ...
    firest_connect(){
        if(!this.firest_connect_state){
            this.render()
            this.handel_custom_scroll_event_emit()
            ...
            <right-div>
                ...
                <bottom-div class='center-v'>
                    ...
                    <div class='files_typs_aria center-v' >
                        ...
                        <c-icon src='icons/csv.svg' size=50 save_type='.csv' class='hid'></c-icon>
                    </div>
    ...
    push(chunk){
        ...
        this.set_data_to_viow_data_For_firest_time(chunk)
    }

    handel_custom_scroll_event_emit(){
        this.table=this.children[0].children[0]
        this.table.addEventListener('wh',()=>{
            if (this.table.offsetHeight + this.table.scrollTop >= this.table.scrollHeight) {
                console.log('bottom')
            }               
        })
    }

    set_data_to_viow_data_For_firest_time(chunk){
        if(this.viow_data.length<200){
            var ammout_to_complite_viow_data=200-this.viow_data.length;
            this.viow_data.push(...chunk.chunk(ammout_to_complite_viow_data))
            this.render_table_data_chunk(this.viow_data)           
        }else{
            return
        }
    }
    ...
    show_hidden_file_extention(){
        ...
        .addEventListener('click',()=>{
            if(!this.files_extintions_visbailty_state){
                this.children[1].children[1].children[1].style.height='60px';
    ...
    save(path,type){
        switch(type){
            case '.xlsx':
                //MML.set_save_data_and_show_save_allert(path,this.result_files_count,0)
    ...
    render_table_data_chunk_text(chunk){
    ...
    render_table_data_chunk_html(chunk){
        for(var i=0;i<chunk.length;i++){
            var tr=document.createElement('tr');
            for(var key of this.table_keys){
                var data=chunk[i][key]||"_";
